...
manyAccum accum (Parser p)
  = Parser (\state ->
    let walk xs state          = walkReply xs state (parserReply (p state))
        walkReply xs state r   = case r of
                                   Ok x state' err  -> let ys = accum x xs
                                                       in seq ys (walk (accum x xs) state')
    ...
    in case (p state) of
         ...
         Consumed reply -> Consumed $ walkReply [] state reply)
