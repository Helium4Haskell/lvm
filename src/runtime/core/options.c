...
static wsize_t heap_wsize_init            = 1*Mega; /* Default initial size of the major heap (words); same constraints as for Heap_chunk_def. */
 /* Setting this to 2*Mega will invalidate some programs (correct/File1) and also (correct/LargeFileOnMac). Setting this initial size to 4*Mega will make LargeFileOnMac go right, but correct/File1 only works for heaps less than 8 MB. TODO JUR */
