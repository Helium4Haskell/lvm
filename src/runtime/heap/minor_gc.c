...
#include "heap.h"
#include "roots.h"
#include "core/stats.h"    /* start/end_gc */
#include "core/signals.h"  /* [force_major_slice] */
...
void set_minor_heap_size (asize_t bsize)
{
  char *new_heap;
  value **new_table;

  Assert (Wsize_bsize(bsize) >= Minor_heap_min_wsize);
  Assert (Wsize_bsize(bsize) <= Minor_heap_max_wsize);
  Assert (bsize % sizeof (value) == 0);
  if (young_ptr != young_end) minor_collection ();
                                           Assert (young_ptr == young_end);
  new_heap = (char *) stat_alloc (bsize);
  if (young_start != NULL){
    stat_free (young_start);
  }
  young_start = new_heap;
  young_end = new_heap + bsize;
  young_limit = young_start;
  young_ptr = young_end;
  minor_heap_size = bsize;
...
{                                 Assert (ref_table_ptr == ref_table_limit);
  ...
  if (ref_table_limit == ref_table_threshold){
    ...
    gc_message (0x08, "Growing ref_table to %s\n", (long)sz/Kilo);
