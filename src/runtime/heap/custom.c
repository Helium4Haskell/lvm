...
{
  raise_internal("equal: abstract value");
  return 0;
}
...
{
  raise_internal("output_value: abstract value");
}
...
/* DAAN: removed default int32/64/native ops */
/*
...
extern struct custom_operations fixed_block_ops;
void init_custom_operations(void)
{
  register_custom_operations(&bytes_ops);
  register_custom_operations(&fixed_block_ops);
}